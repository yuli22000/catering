...
class CartController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $cart = Cart::where('product_id', $request->product_id)->where('name', $request->name);
        ...
        if ($cart->count()) {
            ...
            //sum price * quantity
            $price = $request->price + $request->harga * $cart->qty;
            $totalPrice = 0;

            //get all cart items for the logged in user
            $cartItems = Cart::where('user_id', $request->user()->id)->get();

            //iterate through each cart item and add its total_price to the total price
