...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        return array_merge(parent::share($request), [
            ...
            'countQty' => $request->user() ? [
                'qty'     => \App\Models\Cart::where('user_id', $request->user()->id)->sum('qty') ?? 0,
            ] : null,
            'dilarang' => $request->user() ? [
                'shiping_status'     => \App\Models\Transaction::where('user_id', $request->user()->id)->where('shiping_status') ?? 0,
            ] : null
        ]);
    }
}
